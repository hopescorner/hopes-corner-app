-- Under-18 eligibility: children must be younger than 18 (max 17).
-- Updates both family RPCs (validation, teen-card counting, age groups)
-- and migrates existing child rows to the renamed teen group.
update public.holiday_children
set age_group = 'teen_16_17'
where age_group = 'teen_16_18';

-- Tighten the age check constraint. NOT VALID keeps any pre-existing
-- rows untouched while enforcing the rule for all new writes.
