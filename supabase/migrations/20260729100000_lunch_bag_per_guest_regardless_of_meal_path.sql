-- Lunch bag counts drifted below the number of guests served because the
-- automatic "one bag per person per service day" add was only attempted on the
-- single transition where a guest's *first guest meal* row was created
-- (is_new_guest_meal). Any guest whose day did not pass through exactly that
-- transition silently never got a bag:
--
--   * Guest's first meal of the day is logged as an extra (p_extra = true) —
--     the extra branch never touched lunch bags at all.
--   * The guest-meal row already existed (created by the Services page flow,
--     an import, or a reconcile) so the check-in add took the UPDATE branch and
--     skipped the bag.
--   * auto_meal_additions_enabled was off at the moment of the first meal and
--     turned on later in the day.
--
-- Each of those leaves a permanent one-bag hole for that guest, which is the
-- "always a little delta" between lunch bags and guests served.
--
-- Fix: attempt the bag on every meal command for the guest/day, not just on the
-- first-guest-meal transition. The deduplication_key is uniquely indexed, so
-- repeated attempts collapse to at most one bag per guest per service day and
-- the operation stays idempotent across devices and across the client-side
-- Services flow.
...
begin
  ...
  -- excluded, matching the client-side rule.
